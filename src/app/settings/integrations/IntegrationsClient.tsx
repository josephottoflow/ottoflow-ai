...
export default function IntegrationsClient({
  ...
  const [destByAccount, setDestByAccount] = useState<Record<string, DestinationLite[]>>({});
  ...
  const meta = accounts?.find((a) => a.provider === "meta") ?? null;
  ...
  const discoverDestinations = async (id: string) => {
    ...
    try {
      ...
      setDestByAccount((prev) => ({ ...prev, [id]: j.destinations }));
  ...
  return (
    <div className="p-6 max-w-[900px] mx-auto">
            ...
            {linkedin ? (
              <div className="space-y-4">
                ...
                <DestinationsSection
                  ...
                  list={destByAccount[linkedin.id] ?? null}
                  onDiscover={discoverDestinations}
                  busy={busy}
                />
      ...
      <div className="glass rounded-2xl p-5 mt-4">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-[rgba(24,119,242,0.14)] text-[#5b9bff]">
              <Facebook size={18} />
            </div>
            <div>
              <div className="text-white font-semibold">Meta (Facebook &amp; Instagram)</div>
              <div className="text-white/40 text-sm mt-0.5">
                Connect once to manage Facebook Pages and linked Instagram Business
                accounts. Publishing arrives in a later phase.
              </div>
            </div>
          </div>
          {accounts === null ? (
          ...
          <div className="mt-4 pt-4 border-t border-white/[0.06]">
            {meta ? (
              <div className="space-y-4">
                <div className="text-sm text-white/60">
                  Connected as{" "}
                  <span className="text-white/90">{meta.account_name ?? meta.account_id}</span>
                </div>
                <DestinationsSection
                  accountId={meta.id}
                  list={destByAccount[meta.id] ?? null}
                  onDiscover={discoverDestinations}
                  busy={busy}
                />
                <button
      ...
      <p className="text-white/30 text-xs mt-6">
        More integrations (X, YouTube, Gmail) are coming in later phases.
      </p>
    </div>
  );
}
